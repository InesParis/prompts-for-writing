...
function generatePrompt(event) {
  ...
  let prompt = `generate a writing prompt with the ${topicInput} using the information to set the scene, include ${fictionNonfictionInput} to decide with the prompt should make the writer write a fictional story or a nonfictional one and finally use ${topicsYouLoveInput} to adapt the writing prompt to what the writer really likes.the prompt shouldn't be longer that three sentences. include a first sentence to their writing to help them start.`;
  let context =
    "You are a great English teacher that wants their students to achieve their maximum capacities and you want to help them in particular with their writing skills, generating prompts that are adapted to their needs and fun, the prompts should be short and give and encouraging first line in a new paragraph for the students to continue";
